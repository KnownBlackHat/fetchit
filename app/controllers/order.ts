...
export async function placeOrder(req: Request, res: Response) {
  ...
  const ids = parsedData.data.map((item) => item.id);
  ...
  for (const item of items) {
    const requestedItem = parsedData.data.find((x) => x.id === item.id);
  ...
  const order = await client.$transaction(async (tx) => {
    const usrAddr = await tx.userAddress.findUnique({
      where: {
        userId: req.usrId,
      },
    });
    ...
    for (const item of items) {
      await tx.inventory.update({
        ...
        data: {
          count: {
            decrement: orderItems.find(
              (x) => x.inventory.connect.id === item.id,
            )!.quantity,
          },
        },
      });
